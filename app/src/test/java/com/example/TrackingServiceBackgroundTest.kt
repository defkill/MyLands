...
class TrackingServiceBackgroundTest {
    ...
    @Test
    fun `test TrackingService and ViewModel share singleton sensor instances`() {
        val context = ApplicationProvider.getApplicationContext<Context>()
        ...
        // 1. Verify singleton instance consistency across repeated calls
        assertSame("LocationTracker must be a singleton", locationTracker, com.example.sensor.LocationTracker.getInstance(context))
        assertSame("OrientationManager must be a singleton", orientationManager, com.example.sensor.OrientationManager.getInstance(context))
        assertSame("StepDetectorManager must be a singleton", stepDetectorManager, com.example.sensor.StepDetectorManager.getInstance(context))

        // 2. Verify TrackingService initializes with the exact same singleton references in onCreate
        val serviceController = Robolectric.buildService(TrackingService::class.java)
        val service = serviceController.create().get()

        assertSame("TrackingService must use the shared LocationTracker singleton", locationTracker, service.testLocationTracker)
        assertSame("TrackingService must use the shared OrientationManager singleton", orientationManager, service.testOrientationManager)
        assertSame("TrackingService must use the shared StepDetectorManager singleton", stepDetectorManager, service.testStepDetectorManager)
